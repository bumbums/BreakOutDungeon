...
function startBall2(){
	...
	//ball2 패들
	else if (y2 + dy2 > canvas.height - (paddleHeight+5)) {
...
function startBall3(){
	...
	//ball2 패들
	else if (y3 + dy3 > canvas.height - (paddleHeight+5)) {
...
function startBall4(){
	...
	//ball2 패들
	else if (y4 + dy4 > canvas.height - ballRadius) {
	...
	for (var c = 0; c < brickColumnCount; c++) {
		for (var r = 0; r < brickRowCount; r++) {
			if (bricks[c][r].status >= 1) {//체력변수에맞게
				...
				ctx.beginPath();
				ctx.rect(brickX, brickY, brickWidth, brickHeight);
				if(stageCount==1){ctx.fillStyle = "LIGHTGREEN";}
...
function drawBossLife() {
  if (bossLife >= 1) {
	...
	let hpp = new Image(); 
		hpp.src = "heart.png";
		ctx.drawImage(hpp,bossX+15, bossY-17, 15, 15);
    ctx.fillStyle = "white";
...
function drawCombo() {
	...
	ctx.fillStyle ="#fff";
	ctx.fillText(combo+" COMBO", 10,102);
}
